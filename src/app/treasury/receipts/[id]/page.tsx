...
export default function ReceiptVoucherPage() {
  ...
  const partyName   = isRTL
    ? voucher.customerNameAr
    : (voucher.customerNameEn || voucher.customerNameAr);
  ...
  const voucherPdf: any = {
    ...
    notes: voucher.notes,
    isReceipt: true, isRTL,
    labels: {
      title: t("cashReceiptVoucher"), date: t("date"),
      partyLabel: t("receivedFrom"), accountLabel: t("account"),
      paymentMethodLabel: t("paymentMethod"), referenceLabel: t("reference"),
      amountLabel: isRTL ? "المبلغ المستلم" : "Amount Received",
    ...
    <div className="min-h-screen bg-[color:var(--ink-50)] p-4" dir={isRTL ? "rtl" : "ltr"}>
      ...
      <div className="print:hidden flex items-center justify-between mb-6 max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          <PdfDownloadButton
            ...
            fileName={`receipt-${voucher.voucherNumber}.pdf`}
      ...
      <div className="max-w-2xl mx-auto surface-card print:shadow-none print:border-none p-8">
        ...
        <CompanyPrintHeader
          ...
          isRTL={isRTL}
          documentTitle={t("cashReceiptVoucher")}
          periodLine={voucher.voucherDate}
        />

        {/* ── Company Header ── */}
        <div className="flex justify-between items-start pb-5 mb-5 border-b-2 border-indigo-600 print:hidden">
          <div>
            <h1 className="text-xl font-extrabold text-[color:var(--ink-900)]">{companyName}</h1>
            {branchName && <p className="text-sm text-indigo-600 font-medium mt-0.5">{branchName}</p>}
          ...
          <div className="text-end">
            <div className="inline-block bg-indigo-600 text-white px-4 py-1.5 rounded-lg mb-2">
              <p className="text-sm font-bold">{t("cashReceiptVoucher")}</p>
            </div>
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-3 mb-6 text-sm">
          <InfoRow label={t("receivedFrom")} value={partyName || "—"} />
          ...
          {voucher.customerPhone && (
            <InfoRow label={lang === "ar" ? "هاتف العميل" : "Customer Phone"} value={voucher.customerPhone} />
          )}
        </div>

        {/* ── Amount Box ── */}
        <div className="voucher-amount-box bg-indigo-50 border-2 border-indigo-300 rounded-xl p-6 mb-6 text-center">
          <p className="text-xs font-medium text-indigo-600 uppercase tracking-widest mb-2">
            {lang === "ar" ? "المبلغ المستلم" : "Amount Received"}
          </p>
          <p className="text-4xl font-extrabold text-indigo-800">{fmt(voucher.amount ?? 0)}</p>
        </div>
