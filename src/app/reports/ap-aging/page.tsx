  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-[color:var(--ink-900)]">{t("apAgingTitle")}</h1>
          <p className="text-xs text-[color:var(--ink-500)] mt-0.5">{rows.length} {t("suppliersCount")}</p>
        </div>
      </div>
        ...
        : (
          <div className="overflow-x-auto">
            <table className="w-full zebra-table text-sm">
              ...
              <tbody>
                {rows.map((r: any) => (
                  <tr key={r.supplierId} className="border-t border-[color:var(--ink-100)] hover:bg-[color:var(--brand-50)]/40">
                    <td className="px-4 py-2.5 font-medium text-[color:var(--ink-900)]">{isRTL ? r.supplierName : (r.supplierNameEn || r.supplierName)}</td>
