...
export default function ProductionCostReportPage() {
  ...
  const loading = !companyId || summary === undefined || recipes === undefined;
  ...
  const kpiCards = [
    { label: isRTL ? "إجمالي الإنتاج"      : "Total Produced",     value: kpis?.totalProduced.toFixed(1) ?? "0", suffix: isRTL ? "وحدة" : "units", icon: Package,      color: "#7c3aed" },
    { label: isRTL ? "إجمالي تكلفة المواد" : "Total Material Cost", value: formatCurrency(kpis?.totalMaterialCost ?? 0),                             icon: TrendingUp,   color: "var(--brand-700)" },
    { label: isRTL ? "متوسط تكلفة الوحدة"  : "Avg Cost / Unit",    value: formatCurrency(kpis?.avgCostPerUnit ?? 0),                                 icon: BarChart2,    color: "#b45309" },
    { label: isRTL ? "أوامر مكتملة"        : "Completed Orders",   value: kpis?.completed ?? 0, suffix: isRTL ? "أمر" : "orders",                   icon: CheckCircle2, color: "#15803d" },
  ];
