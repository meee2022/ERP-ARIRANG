...
import { Clock, Package, AlertTriangle, Search, Warehouse } from "lucide-react";
...
function BucketCard({ bucket, data, isRTL, fmt }: any) {
  ...
  return (
    <div className="rounded-2xl border p-4" style={{ background: b.bg + "60", borderColor: b.color + "30" }}>
      ...
      <p className="text-[22px] font-black tabular-nums" style={{ color: "var(--ink-900)" }}>{data.count}</p>
      <p className="text-[11.5px] font-semibold tabular-nums mt-0.5" style={{ color: "var(--ink-600)" }}>{fmt(data.value)}</p>
...
export default function InventoryAgingPage() {
  ...
  const data = useQuery(api.reports.getInventoryAging, companyId ? { companyId } : "skip");
  ...
  return (
    <PrintableReportPage
      ...
      period={data?.asOfDate ?? new Date().toISOString().split("T")[0]}
      filters={
        !companyId || !data ? undefined : (
          <div>
            {/* Bucket summary */}
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-4">
              {BUCKETS.map((b) => (
                <button key={b.key} onClick={() => setBucket(bucket === b.key ? "" : b.key)}>
                  <BucketCard bucket={b.key} data={data.buckets[b.key]} isRTL={isRTL} fmt={fmt} />
                </button>
              ))}
            </div>
                ...
                <tr key={`${r.itemId}-${r.warehouseId}`}
                  ...
                  <td className="px-[14px] py-[8px] whitespace-nowrap">
                    <span className="font-mono text-[11px] font-bold px-2 py-0.5 rounded"
                      style={{ background: "var(--ink-100)", color: "var(--ink-600)" }}>{r.code}</span>
                  </td>
