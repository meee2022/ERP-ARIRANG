  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-[color:var(--ink-900)]">{t("salesReportTitle")}</h1>
          {report && (
            <p className="text-xs text-[color:var(--ink-500)] mt-0.5">
              {report.invoiceCount} {t("invoiceCount")} — {formatCurrency((report.totalSales ?? 0) / 100)}
            </p>
          )}
        </div>
      </div>
      ...
      {report && (
        <div className="surface-card p-4 flex items-center gap-8 flex-wrap text-sm">
          <div>
            <div className="text-xs text-[color:var(--ink-500)]">{t("totalSales")}</div>
            <div className="text-xl font-bold text-[color:var(--ink-900)] tabular-nums">
              {formatCurrency((report.totalSales ?? 0) / 100)}
            </div>
          </div>
          <div>
            <div className="text-xs text-[color:var(--ink-500)]">{t("invoiceCount")}</div>
              ...
              <thead className="bg-[color:var(--ink-50)] text-[color:var(--ink-600)] text-xs uppercase tracking-wider">
                <tr>
                  {groupBy === "day" && <>
                    ...
                    <th className="px-4 py-3 text-end font-semibold">{t("invoiceCount")}</th>
                    <th className="px-4 py-3 text-end font-semibold">{t("totalSales")}</th>
                  </>}
                  {groupBy === "item" && <>
                  ...
                  {groupBy === "customer" && <>
                    <th className="px-4 py-3 text-start font-semibold">{t("customers")}</th>
                    <th className="px-4 py-3 text-end font-semibold">{t("invoiceCount")}</th>
                    <th className="px-4 py-3 text-end font-semibold">{t("totalSales")}</th>
                  </>}
                </tr>
                ...
                {report.data.map((row: any, i: number) => (
                  <tr key={i} className="border-t border-[color:var(--ink-100)] hover:bg-[color:var(--brand-50)]/40">
                    ...
                    {groupBy === "item" && <>
                      ...
                      <td className="px-4 py-3 text-end tabular-nums">{row.qty?.toFixed(2)}</td>
                      <td className="px-4 py-3 text-end font-semibold tabular-nums">{formatCurrency((row.total ?? 0) / 100)}</td>
                    </>}
                    {groupBy === "customer" && <>
                      <td className="px-4 py-3 text-[color:var(--ink-700)]">{row.customerName}</td>
                      <td className="px-4 py-3 text-end tabular-nums">{row.count}</td>
                      <td className="px-4 py-3 text-end font-semibold tabular-nums">{formatCurrency((row.total ?? 0) / 100)}</td>
                    </>}
                  </tr>
