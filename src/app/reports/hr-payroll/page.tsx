...
export default function HrPayrollReportPage() {
  ...
  return (
    <PrintableReportPage
      ...
      period={String(filterYear ?? "")}
      filters={
        <div className="surface-card p-4 flex gap-3 flex-wrap items-end">
          <div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{ background: "#6b1523" }}>
              {[t("payrollPeriod"), t("employeeCount"), isRTL ? "إجمالي الأساسي" : "Total Basic", t("totalAllowancesHr"), t("totalDeductionsHr"), t("totalNetPay"), t("payrollStatus")].map((h, i) => (
                <th key={i} className={`px-[14px] py-[10px] ${i === 0 || i === 6 ? "text-start" : i >= 4 ? "text-end" : "text-center"} text-[11px] font-bold uppercase tracking-wider whitespace-nowrap`} style={{ color: "rgba(255,255,255,0.85)" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
